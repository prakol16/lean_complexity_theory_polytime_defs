...
lemma unpair'_fst_le (n : ℕ) : (unpair' n).1 ≤ n := by { cases n, { refl, }, exact le_of_lt (unpair'_fst_lt _), }
...
lemma unpair'_snd_le (n : ℕ) : (unpair' n).2 ≤ n := by { cases n, { refl, }, exact le_of_lt (unpair'_snd_lt _), }
